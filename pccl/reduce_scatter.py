...
def recursive_halving_reduce_scatter_mpi(
    ...
    for r in range(rounds):
        ...
        if (rank % (size // (2**r))) < (size // (2 ** (r + 1))):
            ...
            recv_buf = torch.empty(
                (half, block_size), dtype=buf.dtype, device=buf.device
            )
...
def _reduce_scatter(
    ...
    if group is None or isinstance(group, dist.ProcessGroup):
        ...
        request = dist.reduce_scatter_tensor(
    ...
    # Case 2: mpi4py.MPI.Comm
        ...
        if use_pccl_cpp_backend:
            request = pccl_mpi_extension.reduce_scatter_mpi(
                output_tensor, input_tensor, group, "recursive" if use_rh else "ring"
            )
        else:
            if not directly_call_mpi:
                if use_rh:
                    request = recursive_halving_reduce_scatter_mpi(
                        output_tensor, input_tensor, group, async_op
                    )
                else:
                    request = ring_reduce_scatter_mpi(
                        output_tensor, input_tensor, group, async_op
                    )
            else:
...
def reduce_scatter_2D(
    ...
    input_permuted = (
        input_tensor.view(inter_node_group_size, intra_node_group_size, -1)
        .transpose(0, 1)
        .reshape(-1)
    )
    ...
    output_intermediate = torch.empty(
        input_permuted.size(0) // intra_node_group_size,
